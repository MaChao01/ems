	...
	<div id="app">
		...
		<el-row>
		  ...
		  <el-select v-model="devid" style='width:45%' placeholder='请输入设备名称'>
		  		<el-option v-for="(obj,index) in dljarr"  :value="obj.typeid">{{obj.devname}}</el-option>
		  </el-select>
		   </el-col>
		 </el-row>
		<el-row>
		  ...
		  value-format='yyyy-MM-dd HH:mm:ss'> </el-date-picker>
			...
			<el-select v-model="reportid" style='width:45%' placeholder='请输入报岗id'>
				<el-option v-for="(obj,index) in reportarr"  :value="obj.reportid">{{obj.reportid}}</el-option>
			</el-select>
		   </el-col>
		   </el-row>
		   <el-row>
...
<script>
	export default {
		...
		mounted:function(){
			var a = this.baseUrl+"/produceJob/findByDevid"
			// 传递给后端的数据
			this.$axios.post(a,{
				headers: {
					'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'
			...
			var b = this.baseUrl+"/produceJob/findByReportid"
			// 传递给后端的数据
			this.$axios.post(b,{
				headers: {
					'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'
